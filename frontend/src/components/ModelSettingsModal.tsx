...
interface ChinaCloudModel {
  id: string;
  owned_by?: string;
  context_length?: number;
}

type ChinaCloudProvider = 'minimax' | 'deepseek' | 'kimi' | 'gemini' | 'qwen' | 'doubao' | 'zhipu';
...
const QWEN_MODELS = ['qwen3.5-plus', 'qwen3-max'];
const DOUBAO_MODELS = ['doubao-seed-2-0-pro-260215', 'doubao-seed-2-0-lite-260215'];
const ZHIPU_MODELS = ['glm-5', 'glm-4.7'];
...
export function ModelSettingsModal({
  ...
  const [isTestingCloudConnection, setIsTestingCloudConnection] = useState<boolean>(false);
  ...
  const [chinaCloudModels, setChinaCloudModels] = useState<Record<string, string[]>>({});
  const [isLoadingChinaCloudModels, setIsLoadingChinaCloudModels] = useState<boolean>(false);
  const [chinaCloudModelsError, setChinaCloudModelsError] = useState<string>('');
  ...
  const modelOptions: Record<string, string[]> = {
    ...
    minimax: chinaCloudModels.minimax?.length ? chinaCloudModels.minimax : MINIMAX_MODELS,
    deepseek: chinaCloudModels.deepseek?.length ? chinaCloudModels.deepseek : DEEPSEEK_MODELS,
    kimi: chinaCloudModels.kimi?.length ? chinaCloudModels.kimi : KIMI_MODELS,
    gemini: chinaCloudModels.gemini?.length ? chinaCloudModels.gemini : GEMINI_MODELS,
    qwen: chinaCloudModels.qwen?.length ? chinaCloudModels.qwen : QWEN_MODELS,
    doubao: chinaCloudModels.doubao?.length ? chinaCloudModels.doubao : DOUBAO_MODELS,
    zhipu: chinaCloudModels.zhipu?.length ? chinaCloudModels.zhipu : ZHIPU_MODELS,
  ...
  const isChinaCloudProvider = (provider: ModelConfig['provider']): provider is ChinaCloudProvider =>
    provider === 'minimax' || provider === 'deepseek' || provider === 'kimi' ||
    provider === 'gemini' || provider === 'qwen' || provider === 'doubao' || provider === 'zhipu';

  const loadChinaCloudModels = async (provider: ChinaCloudProvider, key: string | null) => {
    if (!key?.trim()) return;
    setIsLoadingChinaCloudModels(true);
    setChinaCloudModelsError('');
    try {
      const data = await invoke<ChinaCloudModel[]>('get_china_cloud_models', {
        provider,
        apiKey: key.trim(),
      });
      const ids = data.map((item) => item.id).filter(Boolean);
      setChinaCloudModels((previous) => ({ ...previous, [provider]: ids }));
      setModelConfig((previous: ModelConfig) => {
        if (previous.provider !== provider || ids.includes(previous.model)) return previous;
        const fallbackModels: Record<ChinaCloudProvider, string[]> = {
        ...
        // Replace only an empty/default selection. Preserve a manually entered
        // model ID even when the provider does not expose it in List Models.
        if (previous.model && !fallbackModels[provider].includes(previous.model)) return previous;
        const cached = JSON.parse(localStorage.getItem('providerModelMap') || '{}')[provider];
        return { ...previous, model: ids.includes(cached) ? cached : ids[0] };
      });
    } catch (err) {
      console.error(`Failed to fetch ${provider} models:`, err);
      setChinaCloudModelsError(String(err));
    } finally {
      setIsLoadingChinaCloudModels(false);
    }
  };

  const refreshChinaCloudModels = () => {
    if (isChinaCloudProvider(modelConfig.provider)) {
      loadChinaCloudModels(modelConfig.provider, apiKey);
    }
  };
  ...
  // Fetch the account's current model catalog. Debouncing avoids a request for
  // every character while the user is typing a new key.
  useEffect(() => {
    if (!isChinaCloudProvider(modelConfig.provider) || !apiKey?.trim()) return;
    const provider = modelConfig.provider;
    const timer = setTimeout(() => loadChinaCloudModels(provider, apiKey), 500);
    return () => clearTimeout(timer);
  }, [modelConfig.provider, apiKey]);
  ...
  }, [models, openRouterModels, builtinAiModels, openaiModels, claudeModels, groqModels, chinaCloudModels, modelConfig.provider]);
  ...
  const testCloudConnection = async () => {
    if (!apiKey?.trim() || !modelConfig.model) {
      toast.error(t('model.missingCloudConfig'));
      return;
    }
  ...
  return (
    <div>
      {!compact && <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold">{lt('Model Settings')}</h3>
      </div>}

      <div className="space-y-4">
        <div>
          <Label>{lt('Summarization Model')}</Label>
            ...
            {modelConfig.provider !== 'builtin-ai' && modelConfig.provider !== 'custom-openai' && (
              <Popover open={modelComboboxOpen} onOpenChange={setModelComboboxOpen} modal={true}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="truncate">
                      {modelConfig.model || lt('Select model...')}
                    </span>
                ...
                <PopoverContent className="w-[250px] p-0" align="start">
                  <Command>
                    <CommandInput placeholder={lt('Search models...')} />
                    <CommandList className="max-h-[300px]">
                      {(modelConfig.provider === 'openrouter' && isLoadingOpenRouter) ||
                       ...
                       (isChinaCloudProvider(modelConfig.provider) && isLoadingChinaCloudModels) ? (
                        ...
                        <>
                          <CommandEmpty>{lt('No models found.')}</CommandEmpty>
                          <CommandGroup>
        ...
        {isChinaCloudProvider(modelConfig.provider) && !compact && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label htmlFor="china-cloud-model-id">{t('model.id')}</Label>
              <Button
                type="button"
                variant="ghost"
                size="sm"
                onClick={refreshChinaCloudModels}
                disabled={isLoadingChinaCloudModels || !apiKey?.trim()}
              >
                <RefreshCw className={cn("mr-2 h-4 w-4", isLoadingChinaCloudModels && "animate-spin")} />
                {t('model.refreshCloud')}
              </Button>
            </div>
            <Input
              id="china-cloud-model-id"
              value={modelConfig.model}
              onChange={(event) => setModelConfig((previous: ModelConfig) => ({
                ...previous,
                model: event.target.value,
              }))}
              placeholder={t('model.manualPlaceholder')}
            />
            <p className="text-xs text-muted-foreground">
              {t('model.cloudHelp')}
            </p>
            {chinaCloudModelsError && (
              <Alert className="border-amber-200 bg-amber-50">
                <AlertDescription className="text-amber-800">
                  {t('model.cloudFallback')}
                </AlertDescription>
              </Alert>
            )}
          </div>
        ...
        {modelConfig.provider === 'custom-openai' && (
          <div className="space-y-4 border-t pt-4">
            <div>
              <Label htmlFor="custom-endpoint">{lt('Endpoint URL *')}</Label>
              <Input
                id="custom-endpoint"
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {lt('Base URL of the OpenAI-compatible API')}
              </p>
            </div>

            <div>
              <Label htmlFor="custom-model">{lt('Model Name *')}</Label>
              <Input
                id="custom-model"
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                {lt('Model identifier to use for requests')}
              </p>
            </div>

            <div>
              <Label htmlFor="custom-api-key">{lt('API Key (optional)')}</Label>
              <Input
                id="custom-api-key"
                type="password"
                value={customOpenAIApiKey}
                onChange={(e) => setCustomOpenAIApiKey(e.target.value)}
                placeholder={lt('Leave empty if not required')}
                className="mt-1"
              />
            </div>
            ...
            <div>
              ...
              >
                <Label className="cursor-pointer">{lt('Advanced Options')}</Label>
              ...
              {isCustomOpenAIAdvancedOpen && (
                <div className="space-y-3 pl-2 border-l-2 border-muted mt-2">
                  <div>
                    <Label htmlFor="custom-max-tokens">{lt('Max Tokens')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="custom-temperature">{lt('Temperature (0.0-2.0)')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="custom-top-p">{lt('Top P (0.0-1.0)')}</Label>
                    <Input
                      id="custom-top-p"
                      type="number"
                      step="0.1"
          ...
          <div>
            ...
            <div className="relative mt-1">
              <Input
                ...
                placeholder={lt('Enter your API key')}
                className="pr-24"
              />
              ...
              <div className="absolute inset-y-0 right-0 pr-1 flex items-center space-x-1">
                {apiKey?.trim() && (
                  <Button
                    ...
                    title={isApiKeyLocked ? lt('Unlock to edit') : lt('Lock to prevent editing')}
                  >
            ...
            <Button
              ...
              disabled={isTestingCloudConnection || !apiKey?.trim() || !modelConfig.model}
              className="mt-3 w-full"
            >
              {isTestingCloudConnection ? (
                <><RefreshCw className="mr-2 h-4 w-4 animate-spin" />{t('model.testing')}</>
              ) : (
                <><CheckCircle2 className="mr-2 h-4 w-4" />{t('model.test')}</>
              )}
            </Button>
          </div>
        )}
        ...
        {modelConfig.provider === 'ollama' && (
          <div>
            ...
            >
              <Label className="cursor-pointer">{lt('Custom Endpoint (optional)')}</Label>
            ...
            {!isEndpointSectionCollapsed && (
              <>
                <p className="text-sm text-muted-foreground mt-1 mb-2">
                  {lt('Leave empty or enter a custom endpoint (e.g., http://x.yy.zz:11434)')}
                </p>
                ...
                {ollamaEndpointChanged && !error && (
                  <Alert className="mt-3 border-yellow-500 bg-yellow-50">
                    <AlertDescription className="text-yellow-800">
                      {lt('Endpoint changed. Please click "Fetch Models" to load models from the new endpoint before saving.')}
                    </AlertDescription>
                  </Alert>
                )}
              </>
        ...
        {modelConfig.provider === 'ollama' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-sm font-bold">{lt('Available Ollama Models')}</h4>
              {lastFetchedEndpoint && models.length > 0 && (
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-muted-foreground">{lt('Using:')}</span>
            ...
            {isLoadingOllama ? (
                      ...
                      <div className="space-y-3">
                        ...
                        >
                          {isDownloading('gemma3:1b') ? (
                            <>
                              <RefreshCw className="mr-2 h-4 w-4 animate-spin" />
                              {lt('Downloading gemma3:1b...')}
                            </>
                          ) : (
                            <>
                              <Download className="mr-2 h-4 w-4" />
                              {lt('Download gemma3:1b (Recommended, ~800MB)')}
                            </>
                          )}
                        </Button>
                        ...
                        {isDownloading('gemma3:1b') && getProgress('gemma3:1b') !== undefined && (
                          <div className="bg-white rounded-md border p-3">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm font-medium text-blue-600">{lt('Downloading gemma3:1b')}</span>
              ...
              <ScrollArea className="max-h-[calc(100vh-450px)] overflow-y-auto pr-4">
                {filteredModels.length === 0 ? (
                  <Alert>
                    <AlertDescription>
                      {lt('No models found matching "{query}". Try a different search term.', { query: searchQuery })}
                    </AlertDescription>
                  </Alert>
                ) : (
                      ...
                      return (
                        ...
                        >
                          <div>
                            ...
                            <span className="text-muted-foreground">{lt('with a size of ')}</span>
                          ...
                          {modelIsDownloading && progress !== undefined && (
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-blue-600">{lt('Downloading...')}</span>
