...
export function AudioBackendSelector({
  ...
  useEffect(() => {
    const loadBackends = async () => {
      try {
        ...
        setError(lt('Failed to load backend options'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBackendChange = async (backendId: string) => {
    try {
      ...
      setError(lt('Failed to change backend. Please try again.'));
    }
  };
  ...
  return (
    <div className="space-y-2">
        ...
        <div className="relative">
          ...
          {showTooltip && (
            <div className="absolute z-10 left-6 top-0 w-64 p-3 text-xs bg-gray-900 text-white rounded-lg shadow-lg">
              <p className="font-semibold mb-1">{lt('Audio Capture Methods:')}</p>
              <ul className="space-y-1">
                {backends.map((backend) => (
                  <li key={backend.id}>
                    <span className="font-medium">{lt(backend.name)}:</span> {lt(backend.description)}
                  </li>
                ))}
              </ul>
              <p className="mt-2 text-gray-300">
                {lt('Try different backends to find which works best for your system.')}
              </p>
            </div>
          )}
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="mt-1 text-xs text-gray-600">{lt(backend.description)}</p>
              </div>
            </label>
          );
        })}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• {lt('Backend selection only affects system audio capture')}</p>
        <p>• {lt('Microphone always uses the default method')}</p>
        <p>• {lt('Changes apply to new recording sessions')}</p>
      </div>
    </div>
  );
}
