  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm relative">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-lg font-semibold text-gray-900">{lt('Summary Language')}</h3>
      </div>
      <p className="text-sm text-gray-600 mb-4">
        {lt('Pin one language as the default for new meetings. Unpinned languages remain as quick-switch options in the summary generator. Auto uses the dominant transcript language.')}
      </p>
          ...
          return (
            ...
            >
              <button
                type="button"
                aria-label={isPinned
                  ? lt('Unpin {language} as default', { language: labelForCode(code) })
                  : lt('Pin {language} as default', { language: labelForCode(code) })}
                aria-pressed={isPinned}
                title={isPinned ? lt('Click to unset as default') : lt('Click to set as default')}
              ...
              <button
                type="button"
                aria-label={lt('Remove {language}', { language: labelForCode(code) })}
      ...
      <p className="text-xs text-gray-400 mt-3">
        {pinned
          ? lt('Default: {language} - click it again to unset. Max 5 quick-switch options.', { language: labelForCode(pinned) })
          : lt('Click any language to set it as your default. Max 5 quick-switch options.')}
      </p>
    </div>
  );
}
