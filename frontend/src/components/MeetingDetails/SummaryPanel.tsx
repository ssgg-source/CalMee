...
export function SummaryPanel({
  ...
  const effectiveLangLabel = summaryLang ? labelForCode(summaryLang) : lt('Auto');
  const isLocalFallbackLanguage = summaryLangStorage === 'local_fallback';
  const autoSubtitle = isLocalFallbackLanguage
    ? lt('Saved on this device for folderless meetings')
    : lt('Uses dominant transcript language');
  ...
  useEffect(() => {
    ...
    const loadSummaryLanguage = async () => {
      try {
        ...
        toast.warning(lt('Could not load saved summary language'), {
          description: lt('Using Auto until meeting metadata can be read.'),
        });
        ...
        try {
          ...
          if (
            ...
            if (saved.storage === 'local_fallback') {
              toast.info(lt('Summary language saved on this device'), {
                description: lt('This meeting has no recording folder, so the preference cannot be written to meeting metadata.'),
              });
            }
            if (request.language) {
          ...
          if (
            ...
            toast.error(lt('Failed to save summary language'));
    ...
    <Popover open={langPickerOpen} onOpenChange={setLangPickerOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          size="icon"
          className="h-9 w-9"
          title={lt('Summary language: {language}{suffix}', { language: effectiveLangLabel, suffix: isLocalFallbackLanguage ? lt(' (saved on this device)') : '' })}
          aria-label={lt('Set summary language')}
        >
          <Languages size={18} />
        </Button>
      </PopoverTrigger>
  ...
  return (
    <div style={{ containerType: 'inline-size' }} className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-3xl border border-violet-100 bg-white shadow-sm">
      {/* Title area */}
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-center flex-1">
            <div className="w-full max-w-md px-8 text-center">
              <div className="inline-block animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-blue-500 mb-5"></div>
              <p className="text-gray-800 font-medium mb-3">
                {getSummaryStatusMessage(summaryStatus)}
              </p>
              <div className="h-2 w-full overflow-hidden rounded-full bg-violet-100">
                <div className="h-full w-2/3 animate-pulse rounded-full bg-violet-500 transition-all duration-700" />
              </div>
              <p className="mt-3 text-xs text-gray-500">
                {t('meeting.longSummaryHint')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {summaryResponse && (
            <div className="fixed bottom-0 left-0 right-0 bg-white shadow-lg p-4 max-h-1/3 overflow-y-auto">
              <h3 className="text-lg font-semibold mb-2">{lt('Meeting Summary')}</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-4 rounded-lg shadow-sm">
                  <h4 className="font-medium mb-1">{lt('Key Points')}</h4>
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm mt-4">
                  <h4 className="font-medium mb-1">{lt('Action Items')}</h4>
                  <ul className="list-disc pl-4">
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm mt-4">
                  <h4 className="font-medium mb-1">{lt('Decisions')}</h4>
                  <ul className="list-disc pl-4">
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm mt-4">
                  <h4 className="font-medium mb-1">{lt('Main Topics')}</h4>
                  <ul className="list-disc pl-4">
              ...
              {summaryResponse.raw_summary ? (
                <div className="mt-4">
                  <h4 className="font-medium mb-1">{lt('Full Summary')}</h4>
