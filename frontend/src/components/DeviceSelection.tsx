  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">{lt('Audio Devices')}</h4>
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="mic-selection" className="w-full">
              <SelectValue placeholder={lt('Select Microphone')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{lt('Default Microphone')}</SelectItem>
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{lt('No microphone devices found')}</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">{lt('Microphone Levels:')}</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="system-selection" className="w-full">
              <SelectValue placeholder={lt('Select System Audio')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{lt('Default System Audio')}</SelectItem>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{lt('No system audio devices found')}</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>{lt('Microphone:')}</strong> {lt('Records your voice and ambient sound')}</p>
        <p>• <strong>{lt('System Audio:')}</strong> {lt('Records computer audio (music, calls, etc.)')}</p>
        {isMonitoring && (
          <p>• <strong>{lt('Mic Levels:')}</strong> {lt('Green = good, Yellow = loud, Red = too loud')}</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>{lt('Tip:')}</strong> {lt('Click "Test Mic" to check if your microphone is working')}</p>
        )}
      </div>
    </div>
  );
