...
export function MeetingTabs(){
  ...
  useEffect(()=>{let live=true;void Promise.all(tabs.flatMap(tab=>[
    ...
    invoke<any>('api_get_transcript_refinement_status',{meetingId:tab.id}).then(job=>{if(live&&job.status==='processing')setTaskProgress(current=>({...current,[tab.id]:{...current[tab.id],ai:job.progress?.percentage||1}}));}).catch(()=>undefined),
  ...
  useEffect(()=>{let disposed=false;const cleanups:Array<()=>void>=[];void Promise.all([
    ...
    listen<any>('transcript-refinement-progress',event=>setTaskProgress(current=>({...current,[event.payload.meetingId]:{...current[event.payload.meetingId],ai:event.payload.percentage||1}}))),
