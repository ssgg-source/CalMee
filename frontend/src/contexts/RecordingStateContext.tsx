...
export function RecordingStateProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    void syncWithBackend().then(backendState => {
      // Floating windows are created after recording-started has fired. Recover
      // their duration polling loop from the backend's authoritative state.
      if (backendState?.is_recording) startPolling();
    });
  }, []);
