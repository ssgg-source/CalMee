...
export function ConfigProvider({ children }: { children: ReactNode }) {
      ...
      try {
        ...
        if (config) {
          ...
          setTranscriptModelConfig({
            provider,
            model: provider === 'none' ? '' : (config.model || 'parakeet-tdt-0.6b-v3-int8'),
  ...
  useEffect(() => {
    const loadAllApiKeys = async () => {
      try {
        const providers = ['claude', 'groq', 'openai', 'openrouter', 'minimax', 'deepseek', 'kimi'];
  ...
  const modelOptions: Record<ModelConfig['provider'], string[]> = {
    ...
    minimax: ['MiniMax-M3', 'MiniMax-M2.7-highspeed', 'MiniMax-M2.7', 'MiniMax-M2.5-highspeed', 'MiniMax-M2.5'],
    deepseek: ['deepseek-v4-flash', 'deepseek-v4-pro'],
    kimi: ['kimi-k3', 'kimi-k2.6', 'kimi-k2.5', 'moonshot-v1-128k'],
