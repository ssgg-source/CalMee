...
export function meetingUrl(meetingId:string,source?:string){const params=new URLSearchParams({id:meetingId});if(source)params.set('source',source);return `/meeting-details?${params.toString()}`;}

export function readMeetingTabs():MeetingTab[]{
  if(typeof window==='undefined')return [];
  try{const value=JSON.parse(sessionStorage.getItem(STORAGE_KEY)||'[]');return Array.isArray(value)?value.filter(item=>item?.id&&item?.title):[];}catch{return [];}
}
