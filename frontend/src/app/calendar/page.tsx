...
type CalendarRangeData={calendars:CalendarCollection[];events:CalendarEvent[]};
...
const cacheCalendarRange=(key:string,data:CalendarRangeData)=>{
  ...
  calendarRangeCache.set(key,data);
  while(calendarRangeCache.size>CALENDAR_CACHE_LIMIT){
    const oldest=calendarRangeCache.keys().next().value;
    if(typeof oldest!=='string')break;
    calendarRangeCache.delete(oldest);
  }
};
const fetchCalendarRange=(start:Date,end:Date,force=false)=>{
  ...
  if(!force){
    ...
    const pending=calendarRangeRequests.get(key);
  ...
  const request=Promise.all([
    ...
    return data;
  }).finally(()=>{if(calendarRangeRequests.get(key)===request)calendarRangeRequests.delete(key);});
  calendarRangeRequests.set(key,request);
  return request;
};
...
export default function CalendarPage(){
  ...
  const calendarColors=useMemo(()=>new Map(calendars.map(item=>[item.id,item.color])),[calendars]);
  const eventsByDay=useMemo(()=>visibleEvents.reduce<Map<string,CalendarEvent[]>>((map,event)=>{const key=localDayKey(event.startAt);const items=map.get(key);if(items)items.push(event);else map.set(key,[event]);return map;},new Map()),[visibleEvents]);
  const meetingsByDay=useMemo(()=>meetings.reduce<Map<string,typeof meetings>>((map,meeting)=>{if(!meeting.calendarEventId||!meeting.meetingStartTime)return map;const key=localDayKey(meeting.meetingStartTime);const items=map.get(key);if(items)items.push(meeting);else map.set(key,[meeting]);return map;},new Map()),[meetings]);
  const colorFor=(event:CalendarEvent)=>calendarColors.get(event.calendarId||'')||'#0A84FF';
  const eventsForDay=(date:Date)=>eventsByDay.get(localDayKey(date))||[];
  const meetingsForDay=(date:Date)=>meetingsByDay.get(localDayKey(date))||[];
  ...
  return <div className="calmee-page min-w-[760px]">
    ...
    <header className="calmee-titlebar grid min-h-[52px] shrink-0 grid-cols-[1fr_auto_1fr] items-center px-3">
      <div className="flex min-w-0 items-center gap-2">
        <button onClick={()=>setSidebarCollapsed(value=>!value)} className="calmee-toolbar-button" title={sidebarCollapsed?t('calendar.expandSidebar'):t('calendar.collapseSidebar')} aria-label={sidebarCollapsed?t('calendar.expandSidebar'):t('calendar.collapseSidebar')}>{sidebarCollapsed?<PanelLeftOpen className="h-[17px] w-[17px]"/>:<PanelLeftClose className="h-[17px] w-[17px]"/>}</button>
        <button onClick={()=>{setCursor(new Date());setSelectedDate(new Date());}} className="h-8 rounded-lg border border-border bg-card px-3 text-[12px] font-medium text-foreground shadow-sm transition hover:bg-accent">{t('common.today')}</button>
        <div className="flex h-8 overflow-hidden rounded-lg border border-border bg-card shadow-sm"><button aria-label={t('calendar.previous')} onClick={()=>move(-1)} className="flex w-8 items-center justify-center border-r border-border transition hover:bg-accent"><ChevronLeft className="h-4 w-4"/></button><button aria-label={t('calendar.next')} onClick={()=>move(1)} className="flex w-8 items-center justify-center transition hover:bg-accent"><ChevronRight className="h-4 w-4"/></button></div>
      </div>
      <h1 className="max-w-[440px] truncate px-6 text-center text-[17px] font-semibold tracking-[-0.015em] text-foreground">{title}</h1>
      <div className="flex min-w-0 items-center justify-end gap-2">
        <div className="flex h-8 rounded-lg bg-muted p-0.5">{(['day','week','month'] as ViewMode[]).map(mode=><button key={mode} onClick={()=>setView(mode)} className={`min-w-[48px] rounded-md px-2.5 text-[12px] transition ${view===mode?'bg-card font-medium text-foreground shadow-sm':'text-muted-foreground hover:text-foreground'}`}>{mode==='day'?t('common.day'):mode==='week'?t('common.week'):t('common.month')}</button>)}</div>
        <button onClick={()=>void sync()} disabled={syncing} className="calmee-toolbar-button" title={t('common.sync')} aria-label={t('common.sync')}><RefreshCw className={`h-4 w-4 ${syncing?'animate-spin':''}`}/></button>
        <button onClick={()=>setInboxCollapsed(value=>!value)} className="calmee-toolbar-button" title={inboxCollapsed?(locale==='zh-CN'?'展开未关联会议':'Show unlinked meetings'):(locale==='zh-CN'?'折叠未关联会议':'Hide unlinked meetings')} aria-label={inboxCollapsed?(locale==='zh-CN'?'展开未关联会议':'Show unlinked meetings'):(locale==='zh-CN'?'折叠未关联会议':'Hide unlinked meetings')}>{inboxCollapsed?<PanelRightOpen className="h-[17px] w-[17px]"/>:<PanelRightClose className="h-[17px] w-[17px]"/>}</button>
      </div>
    </header>
    <div className="grid min-h-0 flex-1 transition-[grid-template-columns] duration-200 ease-out" style={{gridTemplateColumns:`${sidebarCollapsed?'0px':'232px'} minmax(480px,1fr) ${inboxCollapsed?'0px':'320px'}`}}>
      {!sidebarCollapsed&&<aside className="col-start-1 overflow-y-auto border-r border-border bg-muted/45 px-3 py-4">
