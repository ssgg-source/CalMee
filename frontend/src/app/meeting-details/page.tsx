...
function MeetingDetailsContent() {
  ...
  // The workspace, AI harnesses, speaker grouping and word count all require the
  // complete transcript. Keep the first page responsive, then exhaust the local
  // pagination gently in the background instead of blocking initial interaction.
  useEffect(() => {
    if (isLoadingTranscripts || isLoadingMore || !hasMore) return;
    const timer = window.setTimeout(() => { void loadMore(); }, 300);
    return () => window.clearTimeout(timer);
  }, [isLoadingTranscripts, isLoadingMore, hasMore, loadedCount, loadMore]);
  ...
  useEffect(() => {
    ...
    if (metadata) {
      ...
      setMeetingDetails({
        ...
        meeting_start_time: metadata.meeting_start_time,
        meeting_end_time: metadata.meeting_end_time,
  ...
  // The saved summary is an independent derived document. It may arrive after
  // the workspace; a slow/legacy summary read must not block the raw transcript.
