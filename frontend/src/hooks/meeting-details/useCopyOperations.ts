...
export function useCopyOperations({
  ...
  const handleCopyTranscript = useCallback(async () => {
    ...
    const date = `## Date: ${new Date(meeting.meeting_start_time || meeting.created_at).toLocaleDateString()}\n\n`;
  ...
  const handleCopySummary = useCallback(async () => {
    try {
      ...
      const metadata = `**Meeting ID:** ${meeting.id}\n**Date:** ${new Date(meeting.meeting_start_time || meeting.created_at).toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
        hour: '2-digit',
