...
fn main() {
    ...
    // Release builds must bundle a reviewed FFmpeg binary. Source-only checks can
    // opt out because they do not execute or package media conversion.
    if std::env::var("CALMEE_SKIP_FFMPEG_BUNDLE").as_deref() == Ok("1") {
        println!("cargo:warning=Skipping FFmpeg bundling for source-only check");
    } else {
...
fn detect_and_report_gpu_capabilities() {
    ...
    println!("cargo:warning=🚀 Building CalMee for: {}", target_os);
