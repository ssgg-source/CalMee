...
pub(crate) fn focus_main_window<R: Runtime>(app: &AppHandle<R>) {
    if let Some(window) = app.get_webview_window("main") {
        // macOS can restore a window using coordinates from a disconnected
        // display. Treat a window as visible only when a useful portion of it
        // intersects one of the currently available monitors.
        let is_on_screen = (|| -> tauri::Result<bool> {
        ...
        if !is_on_screen {
            log::warn!("Main window was outside the visible desktop; centering it");
            if let Err(error) = window.center() {
                log::error!("Failed to center off-screen main window: {}", error);
            }
        }
