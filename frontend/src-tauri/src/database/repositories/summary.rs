...
impl SummaryProcessesRepository {
    pub async fn update_process_progress(
        ...
        let now = Utc::now();
        let metadata = serde_json::to_string(progress)
            .map_err(|e| sqlx::Error::Protocol(format!("Failed to serialize progress: {}", e)))?;
        sqlx::query(
            "UPDATE summary_processes SET metadata = ?, updated_at = ? WHERE meeting_id = ? AND status IN ('PENDING', 'PROCESSING')",
        )
        .bind(metadata)
        .bind(now)
        .bind(meeting_id)
    ...
    pub async fn create_or_reset_process(
        ...
        sqlx::query(
            ...
            VALUES (?, 'PROCESSING', ?, ?, ?, NULL, NULL, '{"stage":"preparing","percentage":3,"message":"Preparing the transcript and local model…"}')
            ON CONFLICT(meeting_id) DO UPDATE SET
                ...
                result = result,
                error = NULL,
                metadata = '{"stage":"preparing","percentage":3,"message":"Preparing the transcript and local model…"}'
            "#
        )
        .bind(meeting_id)
        .bind(now)
    ...
    pub async fn update_process_completed(
        ...
        sqlx::query(
            ...
            SET status = 'completed', result = ?, updated_at = ?, end_time = ?, chunk_count = ?, processing_time = ?, error = NULL, metadata = NULL, result_backup = NULL, result_backup_timestamp = NULL
            WHERE meeting_id = ?
            "#
        )
        .bind(result_str)
