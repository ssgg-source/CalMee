...
impl SettingsRepository {
    ...
    pub async fn save_transcript_api_key(
        ...
        if matches!(
            provider,
            "localWhisper" | "parakeet" | "funasr" | "qwen3asr"
        ) {
            return Ok(());
        }
        sqlx::query(
            "INSERT INTO transcript_provider_credentials (provider, api_key, updated_at) VALUES ($1, $2, CURRENT_TIMESTAMP) ON CONFLICT(provider) DO UPDATE SET api_key = excluded.api_key, updated_at = CURRENT_TIMESTAMP"
        ).bind(provider).bind(api_key).execute(pool).await?;
    ...
    pub async fn get_transcript_api_key(
        ...
        let api_key = sqlx::query_scalar(
            "SELECT api_key FROM transcript_provider_credentials WHERE provider = $1",
        )
        .bind(provider)
        .fetch_optional(pool)
        .await?;
