...
pub async fn initialize_database_on_startup(app: &AppHandle) -> Result<(), String> {
    ...
    if is_first_launch {
        info!("First launch detected - initializing database before notifying the window");

        // AppState must exist before any frontend command can run. Previously the
        // first-launch branch deferred database creation to onboarding, which left
        // commands such as calendar loading registered without their required
        // managed state.
        let db_manager = DatabaseManager::new_from_app_handle(app)
            .await
            .map_err(|e| format!("Failed to initialize first-launch database: {}", e))?;
        app.manage(AppState { db_manager });
        info!("First-launch database initialized successfully");
