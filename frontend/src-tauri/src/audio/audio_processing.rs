...
pub fn sanitize_filename(name: &str) -> String {
    ...
    // APFS and most supported filesystems limit one path component to 255
    // bytes. Keep room for CalMee's timestamp suffix while truncating only at
    // a valid UTF-8 boundary so long Chinese titles remain safe.
    const MAX_SAFE_NAME_BYTES: usize = 180;
    let mut end = sanitized.len().min(MAX_SAFE_NAME_BYTES);
    while end > 0 && !sanitized.is_char_boundary(end) {
        end -= 1;
    }
    let shortened = sanitized[..end].trim_end_matches([' ', '.']).trim();
...
mod filename_tests {
    ...
    #[test]
    fn long_ascii_names_fit_a_filesystem_component() {
        let result = sanitize_filename(&"a".repeat(500));
        assert!(result.len() <= 180);
    }

    #[test]
    fn long_unicode_names_are_truncated_on_a_character_boundary() {
        let result = sanitize_filename(&"meeting-record".repeat(100));
        assert!(result.len() <= 180);
        assert!(std::str::from_utf8(result.as_bytes()).is_ok());
    }

    #[test]
    fn unusable_names_get_a_safe_fallback() {
