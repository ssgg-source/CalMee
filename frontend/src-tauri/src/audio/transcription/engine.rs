...
pub const LIVE_CAPTION_MODEL: &str =
    "iic/speech_paraformer_asr_nat-zh-cn-16k-common-vocab8404-online";

static LIVE_CAPTION_ENGINE: Lazy<Mutex<Option<Arc<crate::funasr_engine::FunAsrEngine>>>> =
...
pub async fn prepare_live_caption_engine() -> Result<Arc<crate::funasr_engine::FunAsrEngine>, String>
{
    ...
    let mut config = crate::funasr_engine::FunAsrConfig::default();
    ...
    config.batch_size_s = 15;

    let mut guard = LIVE_CAPTION_ENGINE.lock().await;
    let engine = match guard.as_ref() {
        ...
        None => {
            let engine = Arc::new(crate::funasr_engine::FunAsrEngine::new(config));
    ...
    engine
        .load_streaming()
        .await
        .map_err(|error| format!("Failed to load live caption model: {}", error))?;
    Ok(engine)
}
...
pub enum TranscriptionEngine {
    ...
    FunAsr(Arc<crate::funasr_engine::FunAsrEngine>),
...
impl TranscriptionEngine {
    ...
    pub async fn is_model_loaded(&self) -> bool {
        match self {
            ...
            Self::FunAsr(engine) => engine.is_model_loaded().await,
    ...
    pub async fn get_current_model(&self) -> Option<String> {
        match self {
            ...
            Self::FunAsr(engine) => engine.get_current_model().await,
    ...
    match config.provider.as_str() {
        ...
        "funasr" | "qwen3asr" => {
            ...
            engine
                ...
                .await
                .map(|_| ())
                .map_err(|error| format!("Failed to load FunASR: {}", error))
        }
        other => {
            warn!(
...
pub async fn get_or_init_transcription_engine<R: Runtime>(
    ...
    match config.provider.as_str() {
        ...
        "funasr" | "qwen3asr" => {
            info!("🎙️ Initializing FunASR transcription engine");
            let engine = crate::funasr_engine::get_or_init_engine().await?;
            if !engine.is_model_loaded().await {
                engine
                    .load_live()
            ...
            Ok(TranscriptionEngine::FunAsr(engine))
        }
        "localWhisper" | _ => {
