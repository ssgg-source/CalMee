...
pub fn start_transcription_task<R: Runtime>(
    ...
    tokio::spawn(async move {
        ...
        // Model preparation is deliberately asynchronous relative to audio
        // capture. Chunks accumulate in the unbounded input queue while this
        // waits for an existing page prewarm or loads the selected model here.
        let engine_lifecycle_guard = crate::audio::common::acquire_engine_lifecycle_lock().await;
        // Live captions deliberately use CalMee's fixed lightweight model.
        // The full model selected in Settings is reserved for the background
        // transcription that starts after the recording is saved.
        let transcription_engine = match super::engine::prepare_live_caption_engine().await {
            Ok(engine) => TranscriptionEngine::FunAsr(engine),
            Err(e) => {
                error!("Failed to initialize live caption engine: {}", e);
                let _ = app.emit("transcription-error", serde_json::json!({
                    "error": e,
                    "userMessage": "Live captions are unavailable. Audio recording is continuing and the full transcript will be created after the meeting.",
                    "actionable": false
                }));
                return;
            }
        ...
        for worker_id in 0..NUM_WORKERS {
            let engine_clone = match &transcription_engine {
                ...
                TranscriptionEngine::FunAsr(e) => TranscriptionEngine::FunAsr(e.clone()),
                                ...
                                .await
                            {
                                Ok((transcript, confidence_opt, is_partial)) => {
                                    ...
                                    let confidence_threshold = match &engine_clone {
                                        ...
                                        TranscriptionEngine::FunAsr(_) => 0.0,
                                    };
        ...
        TranscriptionEngine::FunAsr(funasr_engine) => {
            match funasr_engine.transcribe_streaming(&speech_samples, false).await {
                Ok(result) => {
                    ...
                    info!(
                        "FunASR streaming transcription complete for chunk {} in {} ms: '{}'",
                        chunk.chunk_id, result.elapsed_ms, text
                    );
                    Ok((text, None, true))
                }
                Err(error) => {
                    error!(
                        "FunASR transcription failed for chunk {}: {}",
