...
/// Preserve FunASR's sentence timestamps and make multi-speaker results visible in
/// The legacy text-only transcript schema inherited by CalMee.
pub(crate) fn funasr_result_to_transcripts(
    result: &crate::funasr_engine::FunAsrResult,
    ...
    if result.segments.is_empty() {
        ...
        return if text.is_empty() {
            ...
            vec![(text.to_string(), 0.0, duration_seconds * 1000.0)]
        };
    }
    ...
    result
        ...
        .filter_map(|segment| {
            ...
            let text = if result.speaker_count > 1 {
            ...
            Some((text, segment.start_ms as f64, segment.end_ms as f64))
        })
        .collect()
}
