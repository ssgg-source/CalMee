...
use crate::audio::decoder::{
    decode_audio_file, decode_to_whisper_format_ffmpeg, probe_audio_duration,
};
...
pub async fn start_import<R: Runtime>(
    ...
    let use_parakeet = transcribe && provider.as_deref() == Some("parakeet");
    let use_funasr = transcribe && matches!(provider.as_deref(), Some("funasr" | "qwen3asr"));
    ...
    if transcribe && !use_funasr {
        super::common::unload_engine_after_batch(use_parakeet).await;
    }
    // FunASR stays resident for subsequent recordings and is released on app exit,
    // explicit unload, or a model-changing configuration update.
...
async fn run_import<R: Runtime>(
    ...
    info!(
        "Starting import for '{}' from {} (transcribe={}, language={:?}, model={:?}, provider={:?})",
        title, source_path, transcribe, language, model, provider
    );
    ...
    // Import and transcription are intentionally separate product actions.
    // The import dialog creates a meeting with its original audio, then the
    // meeting workspace owns the single canonical ASR settings/progress flow.
    if !transcribe {
        ...
        emit_progress(&app, "saving", 85, "Saving imported audio...");
        if let Err(error) = write_transcripts_json(&meeting_folder, &segments) {
            warn!("Failed to write empty transcripts.json: {}", error);
        }
        ...
        emit_progress(&app, "complete", 100, "Audio imported");
    ...
    let (audio_samples, mut duration_seconds) = if use_funasr {
        ...
        let duration = tokio::task::spawn_blocking(move || probe_audio_duration(&path_for_probe))
            .await
            .map_err(|e| anyhow!("Audio metadata task join error: {}", e))?
        ...
        let converted = tokio::task::spawn_blocking(move || {
            decode_to_whisper_format_ffmpeg(&path_for_decode, Some(resample_progress))
        })
        .await
        .map_err(|e| anyhow!("Audio conversion task join error: {}", e))??;
        converted
    };
    ...
    let funasr_transcripts = if use_funasr {
        ...
        let progress_callback = std::sync::Arc::new(move |progress: u32, message: &str| {
            let overall_progress = 18 + ((progress as f32 * 0.64) as u32);
            emit_progress(
                &app_for_funasr,
                "transcribing",
        ...
        let result = engine
            .transcribe_file(&dest_path, Some(progress_callback))
            .await
    ...
    let mut all_transcripts: Vec<(String, f64, f64)> = funasr_transcripts.unwrap_or_default();
    ...
    if !funasr_speaker_embeddings.is_empty() {
...
async fn create_meeting_with_transcripts(
    ...
    // Insert meeting
    let meeting_start = recorded_at
        .and_then(|value| chrono::DateTime::parse_from_rfc3339(value).ok())
        .map(|value| value.with_timezone(&chrono::Utc))
        .unwrap_or(now);
    let meeting_end = meeting_start
        + chrono::Duration::milliseconds((duration_seconds.max(0.0) * 1000.0).round() as i64);
    sqlx::query(
        "INSERT INTO meetings (id, title, created_at, updated_at, folder_path, meeting_start_time, meeting_end_time)
         VALUES (?, ?, ?, ?, ?, ?, ?)",
    )
    .bind(&meeting_id)
    .bind(title)
    ...
    match file_path {
        Some(path) => {
            ...
            match validate_audio_file(Path::new(&path_str)) {
                Ok(info) => {
                    // Files selected through the native dialog live outside the static
                    // `$APPDATA/**` asset-protocol scope. Grant access only to this exact
                    // user-selected file so the WebView audio element can play MP3/M4A/etc.
                    app.asset_protocol_scope()
                        .allow_file(&path_str)
                        .map_err(|error| {
                            format!("Unable to authorize audio playback: {}", error)
                        })?;
