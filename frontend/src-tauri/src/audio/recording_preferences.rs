...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\calmee-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("calmee-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("calmee-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/calmee-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("calmee-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/calmee-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("calmee-recordings")
    }
...
pub async fn select_recording_folder<R: Runtime>(
    ...
    app.dialog()
        .file()
        .set_directory(preferences.save_folder)
        .pick_folder(move |path| {
            let _ = sender.send(path);
        });
    let selected = receiver
        .await
        .map_err(|_| "The recording folder picker closed unexpectedly.".to_string())?;
