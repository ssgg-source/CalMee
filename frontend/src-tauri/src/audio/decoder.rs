...
const FFMPEG_ONLY_EXTENSIONS: &[&str] = &[
    "mkv", "webm", "wma", "opus", "mov", "aiff", "aif", "caf", "m4b", "mpeg", "mpg", "avi", "3gp",
    "amr", "ac3", "ape",
];
...
/// Read audio duration from container metadata without decoding sample data.
pub fn probe_audio_duration(path: &Path) -> Result<f64> {
    ...
    let file = std::fs::File::open(path)
        .map_err(|error| anyhow!("Failed to open audio file: {}", error))?;
    ...
    let probed = symphonia::default::get_probe()
        ...
        .map_err(|error| anyhow!("Failed to probe audio format: {}", error))?;
    let track = probed
        .format
        .tracks()
        .iter()
...
/// Decode directly to 16kHz mono f32 PCM with FFmpeg.
///
/// Long stereo recordings can otherwise create multiple gigabytes of
/// source-rate, mono and resampled intermediate buffers.
pub fn decode_to_whisper_format_ffmpeg(
    path: &Path,
    progress_callback: Option<ProgressCallback>,
) -> Result<(Vec<f32>, f64)> {
    use std::io::Read;
    ...
    let ffmpeg_path = find_ffmpeg_path()
        .ok_or_else(|| anyhow!("FFmpeg is required to preprocess imported audio"))?;
    if let Some(callback) = &progress_callback {
        callback(0, "Converting to 16kHz mono audio...");
    }
    ...
    let mut stdout = child
        .stdout
        .take()
        .ok_or_else(|| anyhow!("FFmpeg output was unavailable"))?;
    ...
    loop {
        let bytes_read = stdout
            ...
            .map_err(|error| anyhow!("Failed to read FFmpeg audio: {}", error))?;
        if bytes_read == 0 {
            break;
        }

        pending.extend_from_slice(&read_buffer[..bytes_read]);
        let complete_bytes = pending.len() - (pending.len() % 4);
        samples.extend(pending[..complete_bytes].chunks_exact(4).map(|bytes| {
    ...
    let output = child
        .wait_with_output()
        .map_err(|error| anyhow!("Failed to finish FFmpeg conversion: {}", error))?;
    if !output.status.success() {
        return Err(anyhow!(
    ...
    if samples.is_empty() {
        return Err(anyhow!("FFmpeg returned no audio samples"));
    }
    if !pending.is_empty() {
        return Err(anyhow!("FFmpeg returned an incomplete PCM sample"));
    }
    let duration_seconds = samples.len() as f64 / 16_000.0;

    if let Some(callback) = &progress_callback {
        callback(100, "Audio conversion complete");
    }
    info!(
        "FFmpeg direct conversion complete: {:.2}s, {} samples at 16kHz mono",
        duration_seconds,
        samples.len()
    );
