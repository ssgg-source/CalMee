...
pub struct AudioPipeline {
    ...
    stream_audio_cursor_seconds: f64,
...
impl AudioPipeline {
    pub fn new(
        ...
        Self {
            ...
            sample_rate,
            chunk_id_counter: 0,
            stream_audio_cursor_seconds: 0.0,
            // Performance optimization: reduce logging frequency
    ...
    pub async fn run(mut self) -> Result<()> {
                ...
                Ok(Some(chunk)) => {
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            let stream_duration =
                                mixed_with_gain.len() as f64 / self.sample_rate as f64;
                            let transcription_chunk = AudioChunk {
                                data: mixed_with_gain.clone(),
                                sample_rate: self.sample_rate,
                                timestamp: self.stream_audio_cursor_seconds,
                            ...
                            self.stream_audio_cursor_seconds += stream_duration;
        ...
        // Close the streaming input. The worker finalizes Paraformer's cache;
        // no VAD segments are emitted here because that would duplicate audio.
        self.flush_remaining_audio()?;
    ...
    fn flush_remaining_audio(&mut self) -> Result<()> {
        ...
        if let Err(error) = self.vad_processor.flush() {
            warn!("Failed to reset VAD processor: {}", error);
        }
