...
pub fn get_available_models() -> Vec<ModelDef> {
    vec![
        ...
        // Qwen 3.5 9B - Best local quality tier for machines with 16GB+ RAM.
        ModelDef {
            ...
            layer_count: 40,
            sampling: SamplingParams::qwen35_summary(vec!["<|im_end|>".to_string()]),
            description: "Best built-in Chinese summary quality. About 5.3 GiB; 16GB+ unified memory recommended, with slower generation.".to_string(),
        },
        // Qwen 3.5 0.8B - Very fast draft tier for low-memory machines.
        ModelDef {
            name: "qwen3.5:0.8b".to_string(),
            display_name: "Qwen 3.5 0.8B (Fast Draft)".to_string(),
            gguf_file: "Qwen3.5-0.8B-Q4_K_M.gguf".to_string(),
            template: "qwen3.5_nonthinking".to_string(),
            ...
            layer_count: 24,
            sampling: SamplingParams::qwen35_summary(vec!["<|im_end|>".to_string()]),
            description: "Fastest Chinese draft model at about 0.5 GiB. Suitable for quick extraction, but less reliable for final minutes.".to_string(),
        },
...
mod tests {
    ...
    fn qwen35_models_are_registered_with_expected_metadata() {
        ...
        let qwen_08b = get_model_by_name("qwen3.5:0.8b").expect("qwen 0.8b model should exist");
        assert_eq!(qwen_08b.gguf_file, "Qwen3.5-0.8B-Q4_K_M.gguf");
        assert_eq!(qwen_08b.size_mb, 508);
        assert_eq!(qwen_08b.template, "qwen3.5_nonthinking");
