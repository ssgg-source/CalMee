...
impl SidecarManager {
    ...
    fn resolve_helper_binary() -> Result<PathBuf> {
        ...
        if let Ok(env_path) = std::env::var("CALMEE_LLAMA_HELPER") {
            if !env_path.is_empty() {
                let path = PathBuf::from(env_path);
                if path.exists() {
                    log::info!(
                        "Using llama-helper from CALMEE_LLAMA_HELPER: {}",
                        path.display()
                    );
                    return Ok(path);
                }
        ...
        Err(anyhow!(
            "llama-helper binary not found. Build with 'cd llama-helper && cargo build --release' or set CALMEE_LLAMA_HELPER env var."
        ))
    }
    ...
    pub async fn send_request(&self, request_json: String, timeout: Duration) -> Result<String> {
        ...
        match tokio::time::timeout(timeout, self.read_response()).await {
            ...
            Ok(Err(e)) => {
                // A closed stdout means the native model process is gone. Tear
                // down all stale pipe handles so the next request can restart
                // a clean sidecar instead of repeatedly writing to a dead one.
                log::error!("Sidecar request failed: {}. Resetting process state", e);
                if let Err(shutdown_err) = self.shutdown().await {
                    log::error!("Failed to reset crashed sidecar: {}", shutdown_err);
                }
                Err(e)
            }
            Err(_) => {
