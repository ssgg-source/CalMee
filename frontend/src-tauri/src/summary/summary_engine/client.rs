...
fn runtime_limits(prompt: &str, model_context_size: u32) -> (u32, i32) {
    let prompt_tokens = estimate_prompt_tokens(prompt).max(1);
    // Editing can legitimately return almost as much text as it receives.
    // Short prompts do not need a blanket 4096-token generation allowance.
    let max_tokens = (prompt_tokens + 512).clamp(1_024, models::DEFAULT_MAX_TOKENS as u32);
    let required = prompt_tokens.saturating_add(max_tokens).saturating_add(512);
    let context_size = required
        .saturating_add(4_095)
        .checked_div(4_096)
        .unwrap_or(1)
        .saturating_mul(4_096)
        .clamp(8_192, model_context_size);
    (context_size, max_tokens as i32)
}
...
pub async fn generate_with_builtin(
    ...
    let (runtime_context_size, runtime_max_tokens) =
        runtime_limits(&formatted_prompt, model_def.context_size);
    log::info!(
        "Built-in AI runtime limits: context={}, max_output={} (model supports {})",
    ...
    let request = Request::Generate {
        ...
        context_size: Some(runtime_context_size),
