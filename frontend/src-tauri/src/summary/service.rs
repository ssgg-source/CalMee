...
use crate::summary::processor::{
    ...
    selected_document_template_id, SummaryProgress,
};
...
impl SummaryService {
    ...
    pub async fn process_transcript_background<R: tauri::Runtime>(
        ...
        let token_threshold = if provider == LLMProvider::Ollama {
            ...
            match model {
                Ok(model_def) => {
                    // Keep local prompts comfortably below the advertised
                    // context window. This leaves room for the template and
                    // generated report and avoids native-memory spikes.
                    let optimal = (model_def.context_size.saturating_sub(300) as usize).min(9_000);
                    info!(
        ...
        let progress_callback = move |progress: SummaryProgress| {
            let pool = progress_pool.clone();
            let meeting_id = progress_meeting_id.clone();
            ...
            tauri::async_runtime::spawn(async move {
                ...
                if let Err(error) =
                    SummaryProcessesRepository::update_process_progress(&pool, &meeting_id, &value)
                        .await
                {
                    warn!(
                        "Failed to update summary progress for {}: {}",
                        meeting_id, error
                    );
                }
            });
        ...
        match result {
            Ok((final_markdown, english_markdown, num_chunks)) => {
                ...
                .await
                {
                    ...
                    if let Some(document_template_id) =
                        selected_document_template_id(&custom_prompt)
                    {
                        let now = chrono::Utc::now().to_rfc3339();
                        if let Err(error) = sqlx::query(
                            "INSERT INTO meeting_documents (meeting_id,kind,context_key,markdown,language,template_id,created_at,updated_at) VALUES (?,'meeting_summary','',?,?,?,?,?) ON CONFLICT(meeting_id,kind,context_key) DO UPDATE SET previous_markdown=CASE WHEN meeting_documents.markdown<>excluded.markdown THEN meeting_documents.markdown ELSE meeting_documents.previous_markdown END,markdown=excluded.markdown,language=excluded.language,template_id=excluded.template_id,updated_at=excluded.updated_at",
                        )
                        .bind(&meeting_id)
                        .bind(final_markdown.trim())
                        .bind(summary_language.as_deref().unwrap_or("auto"))
                        .bind(document_template_id)
                        .bind(&now)
                        .bind(&now)
                        .execute(&pool)
                        .await
                        {
                            warn!(
                                "Summary was generated but could not be mirrored to the meeting document: {}",
                                error
                            );
                        }
                    }
                    if let Err(error) = sqlx::query(
                        "UPDATE meeting_record_state SET summary_stale = 0, updated_at = ? WHERE meeting_id = ?",
                    )
                    ...
                    .await
                    {
                        warn!("Failed to mark meeting record summary as current: {}", error);
                    }
            ...
            Err(e) => {
                ...
                if e.contains("cancelled") {
                    ...
                    let _ = _app.emit(
                        "summary-ai-cancelled",
                        serde_json::json!({
                            "meetingId": meeting_id,
                        }),
                    );
                } else {
                    let user_facing_error = if e.contains("Sidecar closed stdout") {
                        "The local AI model process exited unexpectedly, usually because a long transcript created memory pressure. CalMee reset the model process. Try again; the updated workflow processes long transcripts in sections.".to_string()
                    } else {
                        e
                    };
                    Self::update_process_failed(&pool, &meeting_id, &user_facing_error).await;
                    let _ = _app.emit(
                        "summary-ai-error",
                        serde_json::json!({
                            "meetingId": meeting_id,
