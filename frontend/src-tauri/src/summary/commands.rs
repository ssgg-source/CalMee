...
pub async fn api_get_summary<R: Runtime>(
    ...
    match SummaryProcessesRepository::get_summary_data_for_meeting(pool, &meeting_id).await {
        Ok(Some(process)) => {
            ...
            let progress = if status == "pending" || status == "processing" {
                let elapsed_seconds = start_time
                    .map(|started| (chrono::Utc::now() - started).num_seconds().max(0))
                    .unwrap_or(0);
                let elapsed_text = if elapsed_seconds >= 60 {
                ...
                let mut value = process
                    ...
                    .unwrap_or_else(|| {
                        serde_json::json!({
                            "stage": "generating",
                            "percentage": 5,
                            "message": "Local AI is generating meeting notes…"
                        })
                    });
                if let Some(object) = value.as_object_mut() {
                    let base_message = object
                        .get("message")
                        .and_then(|message| message.as_str())
                        .unwrap_or("Local AI is generating meeting notes…")
                        .to_string();
                    object.insert(
                    ...
                    object.insert(
                        "message".to_string(),
                        serde_json::json!(format!("{} ({} elapsed)", base_message, elapsed_text)),
                    );
                }
                Some(value)
            } else {
