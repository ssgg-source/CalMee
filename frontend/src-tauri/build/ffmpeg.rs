...
fn get_ffmpeg_url_for_target(target: &str) -> Result<String, String> {
    if let Ok(url) = std::env::var("CALMEE_FFMPEG_ARCHIVE_URL") {
    ...
    Err(format!(
        "No FFmpeg archive configured for target {target}. Put a verified binary in frontend/src-tauri/binaries or set CALMEE_FFMPEG_ARCHIVE_URL to a ZIP/TAR.XZ archive whose license and checksum have been reviewed."
    ))
}
