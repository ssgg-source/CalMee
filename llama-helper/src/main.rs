...
fn detect_metal_vram() -> Option<f32> {
    ...
    unsafe {
            ...
            && bytes > 0
        {
            ...
            // Apple Silicon uses unified memory. Reserve roughly 30% for
            // macOS and the UI while allowing a 4B Q4 model to fully offload.
            return Some(gb * 0.70);
        }
    }
        ...
        .output()
    {
        if let Ok(stdout) = String::from_utf8(output.stdout) {
            let bytes_str = stdout.rsplit(':').next().unwrap_or(&stdout).trim();
    ...
    // Some sandboxed macOS processes cannot call sysctl, while
    // system_profiler can still report the installed unified memory.
    if let Ok(output) = std::process::Command::new("/usr/sbin/system_profiler")
        ...
        .output()
    {
        if let Ok(value) = serde_json::from_slice::<serde_json::Value>(&output.stdout) {
            let memory = value
                ...
                .and_then(|hardware| hardware.get("physical_memory"))
                .and_then(|memory| memory.as_str());
            if let Some(gb) = memory
...
impl ModelState {
    ...
    fn load_model_if_needed(&mut self, model_path: PathBuf, context_size: u32) -> Result<()> {
        // Contexts are created per request. A context-size change does not
        // require re-reading all model weights from disk.
        if let Some(ref loaded_path) = self.model_path {
            if loaded_path == &model_path {
                if self.context_size != context_size {
                    eprintln!(
                        "✓ Reusing loaded model with {} context (was {})",
