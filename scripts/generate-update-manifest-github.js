...
const githubRepository = process.env.CALMEE_GITHUB_REPOSITORY || process.env.GITHUB_REPOSITORY;
...
if (!githubRepository || !/^[^/]+\/[^/]+$/.test(githubRepository)) {
  console.error('Set CALMEE_GITHUB_REPOSITORY to the CalMee GitHub repository, for example owner/CalMee.');
  process.exit(1);
}
